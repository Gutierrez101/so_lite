...
if __name__ == "__main__":
    print("=== PRUEBA UNITARIA: I/O Manager ===\n")
    
    io_mgr = IOManager()
    io_mgr.initialize()
    
    print("\n1. Crear solicitudes de E/S...")
    print("-" * 50)
    io_mgr.request_io(1, "disk0", "read", 4096, priority=3)
    io_mgr.request_io(2, "disk0", "write", 2048, priority=1)
    io_mgr.request_io(3, "printer0", "write", 1024, priority=5)
    io_mgr.request_io(4, "network0", "read", 8192, priority=2)
    
    print("\n2. Procesar colas de E/S (10 ciclos)...")
    print("-" * 50)
    for i in range(10):
        print(f"\n--- Ciclo {i+1} ---")
        io_mgr.process_io_queues(time.time(), scheduler='PRIORITY')
        time.sleep(0.5)
    
    print("\n3. Estado final de dispositivos...")
    print("-" * 50)
    devices = io_mgr.get_devices_state()
    for dev in devices:
        print(f"{dev['name']}: {dev['status']} - Ops: {dev['total_operations']} - "
              f"Cola: {dev['queue_length']}")
    
    print("\n4. Estadísticas generales...")
    print("-" * 50)
    stats = io_mgr.get_statistics()
